/**
 * The Kilo Code observation canary (T-081; Appendix B A-01, A-02, A-08, A-23, A-24 read for
 * Kilo Code; SRV-19).
 *
 * One call of `handoff_runbooks`, and the run is then read from the probe's observation file:
 * which `clientInfo` Kilo sends, whether the `environment` of the entry arrives whole, whether
 * the two Windows names the pipe is derived from reach a server Kilo starts, in which folder it
 * starts it, and — the fact this surface is keyed on — that the server's parent is the `kilo`
 * process the harness started, with no editor above it. It also records which of Kilo's own
 * variables reach the server: `KILO_CLIENT` is the one T-080 saw only on the VS Code surface.
 * Every one of those is a protocol fact that no sampling of the model changes.
 */
...
import { kiloProjectOf } from './runner.ts';
import { kiloRow, samePath, serverRegistered, type KiloScenario } from './scenario.ts';
...
function cwdIsProject(run: CanaryRun): boolean {
  ...
  return typeof cwd === 'string' && samePath(cwd, kiloProjectOf(run));
}

/** The server's parent, as the probe recorded it. */
function serverParent(run: CanaryRun): number | undefined {
  const ppid = firstObservation(run, 'initialize')?.['ppid'];
  return typeof ppid === 'number' ? ppid : undefined;
}

/** Whether the server's parent is the `kilo` process the harness started itself. */
function parentIsKilo(run: CanaryRun): boolean {
  return run.launched !== undefined && serverParent(run) === run.launched.pid;
}

/** The variables of Kilo's own that reached the server, among those the probe asks about. */
function kiloNames(run: CanaryRun): readonly string[] {
  return envPresent(run).filter((name) => name.startsWith('KILO_'));
}

export const kiloObserveScenario: KiloScenario = {
  id: 'kilo-code-observe',
  title: 'one read-only tool call, and what the server saw of the Kilo Code session',
  ...
  check(run) {
    ...
    const known = kiloRow().client_names;
    ...
    const assertions: Assertion[] = [
      ...
      check(
        ...
        'protocol',
        present.includes('HANDOFF_AGENT') && init?.['agent_id'] === 'kilo-code',
        `env_present ${JSON.stringify(present)}, agent_id ${String(init?.['agent_id'])}`,
      ),
      check(
        'A-08',
        'the handshake carries a clientInfo name the kilo-code row matches',
        'protocol',
      ...
      check(
        'A-24',
        'Kilo starts the server in the folder it was run in',
        'protocol',
        cwdIsProject(run),
      ...
      check(
        'SRV-19',
        "the server's parent is the kilo process the harness started, the key of the CLI surface",
        'protocol',
        parentIsKilo(run),
        `server ppid ${String(serverParent(run))}, kilo pid ${String(run.launched?.pid)}`,
      ),
      ...
      note(
        'SRV-19',
        "the CLI surface names no editor: neither VSCODE_PID nor the extension's KILO_CLIENT arrives",
        !present.includes('VSCODE_PID') && !present.includes('KILO_CLIENT'),
        `Kilo's names that arrived ${JSON.stringify(kiloNames(run))}`,
      ),
      check(
        'model',
  ...
  facts(run) {
    ...
    return {
      ...
      kilo_names_present: kiloNames(run),
      server_cwd_is_project: cwdIsProject(run),
      server_parent_is_kilo: parentIsKilo(run),
