/**
 * The canary driver: `pnpm canary` (T-023, T-066, T-074, T-069, T-072, T-081,
 ...
 * Cursor's editor and its CLI, those of `agents/copilot/` against VS Code and the GitHub
 * Copilot CLI and those of `agents/kilo-code/` against Kilo's CLI, classifies each run, retries
 * a model failure exactly once,
 ...
 * `HANDOFF_CANARY_KILO_CODE_MODEL` the Kilo Code one (default the free auto-router of the Kilo
 * Gateway, see `agents/kilo-code/workspace.ts`), `HANDOFF_CANARY_CODEX`,
 * `HANDOFF_CANARY_OPENCODE`, `HANDOFF_CANARY_CURSOR`, `HANDOFF_CANARY_COPILOT` and
 * `HANDOFF_CANARY_KILO_CODE` name the program when it is not the one on `PATH`,
...
import { KILO_SCENARIOS } from './agents/kilo-code/index.ts';
import { runKilo } from './agents/kilo-code/runner.ts';
import { KILO_DEFAULT_MODEL } from './agents/kilo-code/workspace.ts';
...
const RUNNABLES: readonly Runnable[] = [
  ...
  ...KILO_SCENARIOS.map((scenario): Runnable => ({
    agent: 'kilo-code',
    id: scenario.id,
    title: scenario.title,
    covers: scenario.covers,
    run: () => runKilo(scenario.options),
    check: (run) => scenario.check(run),
    facts: (run) => scenario.facts?.(run) ?? {},
  })),
];
...
async function main(argv: readonly string[]): Promise<number> {
  ...
  const document = {
    ...
    ...(agents.includes('copilot')
      ...
      : {}),
    ...(agents.includes('kilo-code')
      ? { kilo_code_model: process.env['HANDOFF_CANARY_KILO_CODE_MODEL'] ?? KILO_DEFAULT_MODEL }
      : {}),
    scenarios: reports,
